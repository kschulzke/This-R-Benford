...
To get things done in R, we must first load libraries (a.k.a. packages). These act like apps or tools for doing specific tasks like data import, cleansing, transformation, and visualization. We'll include `tictoc` [@R-tictoc] to compare process times for different scripting options.
...
After downloading the [Bitcoin Historical Data](https://www.kaggle.com/datasets/mczielinski/bitcoin-historical-data) from Kaggle, we import it into R Studio using the `data.table` package [@R-data.table] `fread` function, which works much faster on tables containing millions of rows. We could use the `dplyr` package [@R-dplyr] `read_csv` function, which still effective but considerably slower. We'll assign the data to an object called BTC_master.

In the script that follows, the `<-` symbol is the "assignment operator," which assigns the contents of BTC_data.csv to the R object "BTC_master".  We use `tic` and `toc` to measure execution speed, which – for almost 4.9 million rows – approximates 0.25 seconds using `fread`. The speed varies with the hardware (e.g., RAM and CPU specs) and other demands on it.
...
But let's use the combined table in some visualizations. We can start with overlaid histograms.
...
Not exactly overlaid, are they? This is because the Kaggle and Yahoo "volume" distributions are so dissimilar. Hmm.
...
Let's try a couple of additional plots. First, overlaid line plots.
...
This chunk calls *observations* with the two most frequently duplicated Vol_BTC values.
...
We can use the #gets observations starting with 50 or 99
...
#### Calculate EMAD and ExcessMAD

EMAD \<- 1/sqrt(158.8\*n) ExcessMAD \<- bfd.21[["MAD"]] - EMAD[^04-benford-1]

[^04-benford-1]: See Barney & Schulzke (2016), Moderating ‘‘Cry Wolf’’ Events with Excess MAD in Benford’s Law Research and Practice, A77.
...
The My Law concept was first articulated in print by Mark Nigrini in 2012. The idea is that individual organizations may develop, in their data sets, first-digits patterns that do not conform to Benford's Law but are internally consistent over time. Thus, these proprietary digital patterns might be used as a "My Law" in place of Benford's Law. Significant departures from My Law might call for further investigation.[^04-benford-2]

[^04-benford-2]: Mark Nigrini, Benford's Law: Applications for Forensic Accounting, Auditing, and Fraud Detection (2012), pp. 295-300.
...
The first step is to use the benford function to create...
...
MyLaw is a table that combines data from bfd.18_20 and bfd.21.

```{r}
MyLaw <- data.frame(as.character(bfd.18_20$bfd$digits), bfd.18_20$bfd$data.dist, bfd.21$bfd$data.dist, bfd.21$bfd$benford.dist)
head(MyLaw)
```
...
The first plot shows -- for minute-by-minute transaction volumes -- the actual distribution of first-two digits in the Kaggle data (green histogram); the Benford's Law predicted distribution (red dashed line); and the My Law distribution (blue dashed line) derived from Kaggle data for 2018-2020.[^04-benford-3]

[^04-benford-3]: Michael Clark's Benford's ADA analysis submission in ACCT 7530 contributed some snippets and inspired others used in this My Law analysis.
...
ggplot(MyLaw) +
  ...
  labs(y = "Difference in relative frequency",
       x = "First two digits")
